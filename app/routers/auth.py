...
COOKIE_SECURE = COOKIE_SECURE or (COOKIE_SAMESITE == "none")
COOKIE_HTTPONLY = True
COOKIE_DOMAIN = os.getenv("COOKIE_DOMAIN") or None
COOKIE_PATH = "/"

ACCESS_MAX_AGE = int(os.getenv("ACCESS_MAX_AGE", str(60 * 15)))          # 15 min
REFRESH_MAX_AGE = int(os.getenv("REFRESH_MAX_AGE", str(60 * 60 * 24 * 7)))  # 7 giorni
...
# ✅ FIX COOKIE (rimosso expires=max_age)
...
ALT_REFRESH_NAMES = [REFRESH_COOKIE, "sb-refresh-token", "refreshToken", "sb-refreshToken"]
...
def register(payload: RegisterIn):
    ...
    if not USERNAME_RE.fullmatch(uname):
        raise HTTPException(status_code=422, detail="Username non valido. Usa 3-30 caratteri minuscoli, numeri e underscore.")
...
def refresh(request: Request, response: Response, body: Dict[str, Any] | None = None):
    ...
    return TokenOut(access_token=new_access, refresh_token=new_refresh, expires_in=expires_in)
